...
<html lang="de">
    <head>
        ...
        <?php
            ...
            $result3 = $statement->execute(array('email' => $email));
            ...
            if (isset($_GET['id'])) {
                ...
                if($result) {
                    $showFormular = true;
                    if ($_POST['time'] !== '') {
                        $url = 'http://192.168.188.21/relay/0?turn=on&timer='.$_POST['time'];
                        @fopen(strval($url), 'r');
                        $get_door_status = true;
                    } else {
                        $url = 'http://192.168.188.21/relay/0?turn=on&timer=5';
                        @fopen(strval($url), 'r');
                        $get_door_status = true;
                    }
        ...
        <form action="?id=<?php echo $_SESSION['userid']; ?>" method="post">
            <div class="container">
                ...
                <div class="box">
                    ...
                    ?></p>
                    <button class="loginBtn"><a class="btnText" href="account-control.php">Account-Management</a></button>
                </div>
